...
class OmniAttention(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, kernel_size, groups=1, reduction=0.0625, kernel_num=4, min_channel=16):
        ...
        self.insn = nn.InstanceNorm2d(attention_channel,affine=True)
...
class AdaptiveDilatedConv(ModulatedDeformConv2d):
    ...
    def forward(self, x):
        ...
        if hasattr(self, 'OMNI_ATT1') and hasattr(self, 'OMNI_ATT2'):
            ...
            #不区分低频/高频卷积核
            adaptive_weight = adaptive_weight * (2 * c_att.unsqueeze(1)) * (2 * f_att.unsqueeze(2))

            #下面这段内容是关于分别生成低频/高频的卷积核
            # adaptive_weight_mean = adaptive_weight.mean(dim=(-1, -2), keepdim=True)
...
class enhancedResBlock(nn.Module):
    def __init__(self, in_channel, out_channel, norm='backward'):
        ...
        self.main = nn.Sequential(
            AdaptiveDilatedConv(out_channel, in_channel, kernel_size=3),
            nn.ReLU(),
            AdaptiveDilatedConv(out_channel, out_channel, kernel_size=3),
        )
        self.main_fft = nn.Sequential(
            BasicConv_do(out_channel*2, out_channel*2, kernel_size=1, stride=1, relu=True),
            BasicConv_do(out_channel*2, out_channel*2, kernel_size=1, stride=1, relu=False)
